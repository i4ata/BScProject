...
def decisions_step(agents: List[Agent], 
                   envs: List[Rice], 
                   states: List[List[Dict[str, np.ndarray]]]) -> Tuple[List[Dict[str, np.ndarray]], List[float]]:
    ...
    states, rewards = zip(*[env.register_decisions(actions[i]) for i, env in enumerate(envs)])
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for batch in range(batch_size):
            ...
            for t in range(episode_length):
                ...
                states, rewards_negotiation = decisions_step(agents, envs, states)
                ...
                for i, agent in enumerate(agents):
                    for env_id in range(len(envs)):
                        ...
                        agent.proposal_net.buffer.rewards[env_id].append(mean_rewards_action[env_id])
                        agent.proposal_net.buffer.is_terminals[env_id].append(is_terminal)

                        agent.decision_net.buffer.rewards[env_id].append(mean_rewards_action[env_id])
...
def eval_agents_with_comm_deterministic(agents: List[Agent], env: Rice) -> np.ndarray:
    ...
    for step in range(env.episode_length):
        ...
        state, _ = env.register_decisions(decisions)
