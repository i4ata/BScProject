...
class Agent():
    ...
    def __init__(self, state_space: int, action_space: MultiDiscrete, id : int, device : str = 'cpu'):
        ...
        self.proposal_net = PPONegotiation(state_space, action_space.nvec.sum() * 2, device)
        self.decision_net = PPONegotiation(state_space + 4 * action_space.nvec.sum(), 1, device)
