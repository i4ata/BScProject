...
class Actor(nn.Module):
    ...
    def forward(self, 
                ...
                hidden_state: Optional[Tuple[torch.Tensor, torch.Tensor]] = None) -> List[torch.Tensor]:
...
class ActivityNet(nn.Module):
    ...
    def evaluate(self, 
                 ...
                 actor_hidden_state: torch.Tensor,
                 critic_hidden_state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        
        action_logits = self.actor(env_state, action_mask, actor_hidden_state)
        state_values = self.critic(env_state, action_mask, critic_hidden_state)
